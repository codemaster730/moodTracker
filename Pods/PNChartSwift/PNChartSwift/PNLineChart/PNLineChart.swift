...
open class PNLineChart: UIView{
    ...
    */
    ...
    open var chartData: NSArray = []{
        didSet{
            ...
            for layer : AnyObject in chartLineArray as [AnyObject]{
                (layer as! CALayer).removeFromSuperlayer()
            }
            for layer : AnyObject in chartPointArray as [AnyObject]{
                (layer as! CALayer).removeFromSuperlayer()
            }
            ...
            for chart : AnyObject in chartData as [AnyObject]{
    ...
    */
    ...
    func touchPoint(_ touches: NSSet!, withEvent event: UIEvent!){
        ...
        for linePoints:AnyObject in pathPoints as [AnyObject]{
            ...
            for i:NSInteger in 0 ..< (linePointsArray.count - 1){
                ...
                if distance <= 5.0 {
                    ...
                    for path : AnyObject in chartPaths as [AnyObject]{
                        
                        var pointContainsPath:Bool = false
                        if (path as! UIBezierPath).cgPath.contains(p1){
                            pointContainsPath = true
                        }
